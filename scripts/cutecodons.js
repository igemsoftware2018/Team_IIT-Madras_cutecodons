...
function displayOutput(output){
  ...
  totalCount = 0;
  for(i=0;i<64;i++){
    codons[i] = bases[Math.floor(i/16)%4] + bases[i%4] + bases[Math.floor(i/4)%4];
    totalCount += codonCount[codons[i]];
  }
  for(i=0;i<64;i++){
    codon = bases[Math.floor(i/16)%4] + bases[i%4] + bases[Math.floor(i/4)%4];
    if(i%4==0){
      outhtml += '<tr>'
    }
    outhtml += `<td>${codon}:</td><td>${Math.round(1000*codonCount[codon]/totalCount)/10}</td>`;
    if(i%4==3){
      outhtml += '</tr>'
    }
  }
